...
This document describes how `IvanMurzak/Unity-PlayerPrefsEx` signs its
`extensions.unity.playerprefsex` package so the warning no longer appears in
Unity 6.3+.
...
resulting archive contains `package/.attestation.p7m` and that its basename
begins with `extensions.unity.playerprefsex-`, and uploads the signed `.tgz` as
...
```bash
gh secret set UPM_SERVICE_ACCOUNT_KEY_ID     --repo IvanMurzak/Unity-PlayerPrefsEx
gh secret set UPM_SERVICE_ACCOUNT_KEY_SECRET --repo IvanMurzak/Unity-PlayerPrefsEx
gh secret set UPM_ORG_ID                     --repo IvanMurzak/Unity-PlayerPrefsEx
```

### 3. File the OpenUPM listing change

OpenUPM's package listing for `extensions.unity.playerprefsex` may have
...
repository at `data/packages/extensions.unity.playerprefsex.yml`. Open a PR there
changing:
...
ships a `.unitypackage` installer asset and may add more `.tgz` assets later
(e.g. signing a second package), so the prefix guard prevents a future-breaking
failure mode:
...
1. Go to the [release page](https://github.com/IvanMurzak/Unity-PlayerPrefsEx/releases)
   for the new version and confirm an `extensions.unity.playerprefsex-<version>.tgz`
...
2. Inspect the tarball locally to confirm it contains the signing attestation:
   ...
   curl -fsSL -o package.tgz \
     https://github.com/IvanMurzak/Unity-PlayerPrefsEx/releases/download/<version>/extensions.unity.playerprefsex-<version>.tgz
